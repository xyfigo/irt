...
/**
  ...
  */
class MasterModel extends IRTModelBase {
  /**
    * 初始化参数
    *
  ...
  /**
    ...
    */
  override def getParamMeta: (String, String) = ("master", "double")
  ...
  /**
    ...
    * @return link函数值
    */
  override def link(rec: Array[Double], param: Double): Double = rec(0) * (param - rec(1))
  ...
  override def gradientProvider(data: Array[Array[Double]], param: Double): Double = {
    data.map(row => row(0) * (row.last - predict(row, param))).sum
  }
