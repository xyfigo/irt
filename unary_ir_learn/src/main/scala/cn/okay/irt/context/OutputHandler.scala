...
object OutputHandler {
  private def appendParamOutputSchema(model: CompoundModel, schema: StructType): StructType = {
    var newSchema = schema
    for (meta <- model.getParamMetas) newSchema = newSchema.add(meta._1, meta._2, nullable = false)
