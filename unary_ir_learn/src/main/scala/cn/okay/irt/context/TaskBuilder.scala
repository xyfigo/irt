...
import cn.okay.irt.learn.model.pl2.{DifficultyModel, MasterModel, SlopeModel}
...
object TaskBuilder {
  ...
  def initContext(args: Array[String]): SparkSession = {
    SparkSession.builder.master("local[3]").appName("unary_ir_learn")
  ...
  def getStudentModel: CompoundModel = {
    val model = new CompoundModel
    model.subModels = Array(new MasterModel)
    model
  }

  def getQuestionModel: CompoundModel = {
    val model = new CompoundModel
    model.subModels = Array(new SlopeModel, new DifficultyModel)
    model
  }
}
